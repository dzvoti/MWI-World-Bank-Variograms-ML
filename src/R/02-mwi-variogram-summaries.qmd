...
## Directory structure
 ...
 |-----|-------------|
 |scripts| |
 | `src\qgis-python\01-mwi-sample-points-creator.py` | Python script to create sample points in QGIS.   |
 | `src\R\01-mwi-variogram-parallel.R`| R script to calculate the variograms|
 |Inputs|  |
 |`data\admin_inputs\mwi_eas_acho\echo2_prioritization\ECHO2_prioritization.shp` | Shapefile of enumeration areas in Malawi|
 | `data\admin_inputs\mwi_eas_acho\lake_malawi.shp` | Shapefile of Lake Malawi|
 |Outputs|
 |`data\variogram_outputs\mwi_sample_points_variogram.csv` | CSV file with the results of the variogram calculations|
 | `data\point_outputs\mwi_sample_points.shp` | Shapefile of the sample points generated in QGIS|
...
```{python echo=TRUE, message=FALSE, warning=FALSE,eval=FALSE,python.reticulate = FALSE}
...
malawi <- st_read(file.path(wd, "/data/admin_inputs/mwi_eas_acho/echo2_prioritization/ECHO2_prioritization.shp"))

lake_Malawi <- st_read(file.path(wd, "/data/admin_inputs/mwi_eas_acho/lake_malawi.shp"))
...
# Plot the sample points and the polygon shapefile
leaflet() %>%
    ...
    addCircleMarkers(data = sample_points, radius = 1, color = "red", fillOpacity = 0.5)
```
