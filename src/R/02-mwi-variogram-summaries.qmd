...
```{r echo=TRUE, message=FALSE, warning=FALSE,collapse=TRUE,eval=FALSE}
...
:::{.callout-note collapse="true"}
## Variogram summary statistics script
```{r echo=TRUE, message=FALSE, warning=FALSE,eval=FALSE,collapse=TRUE}
...
There are duplicates in the data that are caused by reverse pairwise calculations i.e. A-B and B-A. We will remove these duplicates and keep the first occurence of the pair. 
...
```{r echo=TRUE, message=FALSE, warning=FALSE,eval=TRUE,collapse=TRUE}
#| caption="Results without duplicates"
results_import |>
    mutate(row_id_1 = pmax(p1_id, p2_id), row_id_2 = pmin(p1_id, p2_id)) |>
    distinct(row_id_1, row_id_2, .keep_all = TRUE) |> 
    arrange(row_id_1, row_id_2)|>
